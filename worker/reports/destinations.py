...
def parse_report_targets(raw_targets: str) -> list[str]:
    ...
    if not targets:
        raise ValueError("REPORT_TARGETS must include at least one target: test or production")
...
def get_report_destinations(config: Settings = settings) -> list[ReportDestination]:
    ...
    for target in parse_report_targets(config.REPORT_TARGETS):
        if target == "test":
            ...
            if not group_jid:
                raise ValueError("WHATSAPP_TEST_GROUP_JID is required when REPORT_TARGETS includes test")
        else:
            group_jid = config.WHATSAPP_REPORT_GROUP_JID.strip()
            if not group_jid:
                raise ValueError("WHATSAPP_REPORT_GROUP_JID is required when REPORT_TARGETS includes production")

        if not group_jid.endswith("@g.us"):
            raise ValueError(f"{target} destination must be a WhatsApp group JID ending with @g.us")
