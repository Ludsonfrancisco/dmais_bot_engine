...
async def handle(agendamento: dict) -> dict | None:
    ...
    logger.info("enviar_inicial.start", agendamento_id=agendamento_id, telefone=telefone)
    ...
    if not await evolution_client.check_exists(telefone):
        logger.warning("enviar_inicial.number_not_on_whatsapp",
                       agendamento_id=agendamento_id, telefone=telefone)
    ...
    try:
        await django_client.update_status(
            agendamento_id, "AGUARDANDO_CLIENTE",
            motivo="Mensagem inicial enviada pelo motor",
            inc_tentativas=True,
        )
    except Exception as exc:
        logger.warning("django.status_patch_failed", agendamento_id=agendamento_id, error=str(exc))
