...
async def test_handle_enviar_slots_success():
    ...
    with (
        patch(
            "worker.handlers.enviar_slots.django_client.listar_slots",
            new_callable=AsyncMock,
        ) as mock_listar,
        patch(
            "worker.handlers.enviar_slots.evolution_client.send_text_message",
            new_callable=AsyncMock,
        ) as mock_send_text,
        patch(
            "worker.handlers.enviar_slots.redis_queue.set_state", new_callable=AsyncMock
        ),
    ):
...
async def test_handle_enviar_slots_no_slots():
    ...
    with (
        ...
        patch(
            "worker.handlers.enviar_slots.django_client.post_webhook",
            new_callable=AsyncMock,
        ) as mock_webhook,
        patch(
            "worker.handlers.enviar_slots.redis_queue.clear_state",
            new_callable=AsyncMock,
        ),
    ):
...
async def test_handle_enviar_slots_error_django():
    ...
    with patch(
        "worker.handlers.enviar_slots.django_client.listar_slots",
        side_effect=Exception("Django Error"),
    ):
